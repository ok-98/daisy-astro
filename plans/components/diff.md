...
> **Status:** Planned. Nothing in §4 is implemented. Facts marked **[verified]** were checked on 2026-08-29 against the shipped CSS of `daisyui@5.7.22` (`node_modules/daisyui/components/diff.css`) and the doc page source (`packages/docs/src/routes/(routes)/components/diff/+page.md` in `saadeghi/daisyui`). §3g lists what is **unverified**.
...
1. **Does slot content land as a direct child of each item?** Blocking. `.diff-item-1 > *` and `.diff-item-2 > *` are **child** selectors **[verified]**, and they are what apply `width: 100cqi; height: 100%; position: absolute; object-fit: cover`. An injected wrapper would take those rules itself and leave the real `<img>` at its natural size inside an absolutely-positioned box — so the two halves would no longer line up, which reads as a broken comparison rather than a broken wrapper. Seventh plan to hit this: `plans/components/aura.md` §3e.1, `plans/components/carousel.md` §3g.1, `plans/components/chat-bubble.md` §3f.1, `plans/components/countdown.md` §3f.2, `plans/components/breadcrumbs.md` §3f.1, `plans/components/alert.md` §3d.2. One answer, recorded in all of them.
...
- [ ] **Step 1:** Resolve §3g.1 (direct children of the item wrappers) — blocking, and the shared question across seven plans now. Settle §3g.3 (image URLs, and a matched pair if local) with Avatar/Card/Carousel.
- [ ] **Step 2:** No new shared unions, and no use of the existing ones — there are no variant axes (§1). `variants.ts` untouched. Skip.
- [ ] **Step 3:** Replace the `Diff.astro` dummy scaffold per §4 — the root `<figure>` is already right, the three children and the attributes are the work — then walk the Astro idioms gate.
- [ ] **Step 4:** Replace `Diff.stories.ts` per §5.
- [ ] **Step 5:** `pnpm storybook` from `packages/daisy-astro/`, open `Components/Diff`, verify:
  - `Default` shows a 16:9 box with a vertical divider and a pill handle in the middle.
  - **Dragging the handle reveals the second image**, and the two halves stay aligned — misalignment means §3g.1 (§3g.2 if it will not drag at all).
  - `NoAspectRatio` collapses to a thin strip beside the working copy (§3c). Expected, not a bug.
  - `KeyboardToggle`: tab to the figure → the split jumps near 95%; tab again → near 5% (§3d). Confirm there is no arrow-key adjustment, so the JSDoc's "toggle, not slider" is accurate.
  - `InertContent`: the button inside does not respond to clicks (§3a).
  - Inspect the items in devtools: `role="img"` plus the `aria-label` from `item1Label`/`item2Label` (§3e).
  - If a device or emulator is handy, check iOS Safari once (§3f) — expected: no pill, focus toggle only.
- [ ] **Step 6:** Confirm forwarding via `Passthrough`. Headless check, which also answers §3g.1:
  ```bash
  pnpm build-storybook
  grep -rhoE '<div class="diff-item-1"[^>]*><img' storybook-static/astro-prerendered-stories.json | head
  grep -rhoc 'diff-resizer' storybook-static/astro-prerendered-stories.json
  ```
  The first must match — the `<img>` immediately inside the item, no wrapper between; the second confirms the resizer is emitted for every story.
- [ ] **Step 7:** Update the `Diff` row in `plans/README.md` to **Implemented**, noting the narrow browser support from §3f so it is visible from the checklist.
- [ ] **Step 8:** Commit.

## 7. Acceptance checklist

- [ ] All 4 daisyUI classes from §1 are reachable: `diff` on the root, `diff-item-1`/`diff-item-2` as slot wrappers, `diff-resizer` rendered empty.
- [ ] **No `<script>` and no split-position prop** — the divider is native CSS `resize` (§0).
- [ ] Slot content renders as the **direct child** of each item — checked in the build output, not by eye (§3g.1).
- [ ] `role="img"` on both items and `tabindex="0"` on the figure and on item 1 (§3b, §3d).
- [ ] `item1Label` / `item2Label` reach `aria-label` on their wrappers (§3e).
- [ ] No invented axis — no colour, no size, no `item1Class`/`item2Class` (§2), no `DiffItem` component (§0).
- [ ] JSDoc states: an aspect ratio or height is required (§3c), item 1 is the clipped overlay (§3a), content inside is inert (§3a), the keyboard behaviour is a toggle (§3d), and iOS Safari has no drag (§3f).
- [ ] `Playground` exposes `class` and both label props.
- [ ] One story per doc-page example, reproducing that example's markup and copy, plus `NoAspectRatio`, `KeyboardToggle` and `InertContent`.
