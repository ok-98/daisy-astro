...
1. **Does one slot value render as a real direct child?** Blocking, per §3b. If `@storybook-astro/framework` (or Astro's slot rendering) introduces any wrapper element, the radius selectors and the z-index rule both break. Verify by grepping the rendered HTML for `<div class="aura"><div class="card"` with nothing in between — do not judge it by eye, since the card case looks plausible either way. `plans/components/alert.md` §3d.2 is the same question; whichever component is built first should record the answer in both plans.
2. **Slot sanitization vs the child markup.** The stories pass `div.card` / `button.btn` / inline `<svg>` (pricing example) as slot HTML, and the framework sanitizes with conservative defaults (`plans/README.md` §4). A stripped child leaves an *empty* `.aura`, which still renders a small animated blob — so it looks like a working component with missing content rather than an error.
